...
// Date.UTC was off by 1024 on a Mac. Every product that feeds a sum is
// rounded by an explicit conversion instead. This compiles the module for
// arm64 and fails on any fused instruction a line does not ask for by
// calling math.FMA, so that a machine that never fuses still catches one.
func TestNoFusedArithmetic(t *testing.T) {
	if testing.Short() {
		t.Skip("compiles the module for arm64")
	}
	cmd := exec.Command("go", "build", "-gcflags=github.com/go-quickjs/go-intl/...=-S", "./...")
