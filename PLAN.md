...
| **Compatibility profile** | chosen divergence by divergence, `Standard` and `NodeICU` its two ends; seven named: the narrow space, two-letter tags, the collation keyword, the duration overflow, and, for go-quickjs's standards mode, the twelve-hour cycle, the Islamic eras and Temporal's formats, each tested on both sides |
